    ...
    for name in df.columns:
        ...
        examples = (
            s.dropna().astype(str).unique()[
                :example_values_per_column].tolist()
            if non_null > 0
            else []
        )
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_suspicious_id_duplicates"] = any(("id" in col.name and ((col.unique + col.missing) < summary.n_rows)) for col in summary.columns)

    max_missing_share = float(
        missing_df["missing_share"].max()) if not missing_df.empty else 0.0
