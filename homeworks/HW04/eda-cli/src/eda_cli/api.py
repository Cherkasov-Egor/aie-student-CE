...
class QualityFlagsResponse(BaseModel):
    """Ответ заглушки модели критериев качества датасета"""
    flags: dict[str, bool] = Field(
        ...,
        description="Флаги качества данных, в том числе и добавленные в HW03 самостоятельно"
    )
# ---------- Системный эндпоинт ----------
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Не удалось прочитать CSV: {exc}")
...
# ---------- /quality-flags-from-csv: проверка csv на наши флаги ----------

@app.post(
    "/quality-flags-from-csv", response_model=QualityFlagsResponse, tags=["quality"],
    summary="Проверка csv файла на соответсвия флагам качества датасета"
)
async def quality_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает набор булевых флагов, вычисленных при помощи compute_quality_flags.

    Именно это по сути связывает S04 (HTTP-сервис) и HW04 (потуги на создание эндпоинта).
    """
